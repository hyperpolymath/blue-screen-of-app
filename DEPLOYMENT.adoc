...
* Just command runner
* Deno runtime
* AffineScript compiler (optional, for development)
...
[source,bash]
----
# Build everything (AffineScript + config export)
just build
...
1. Compiles AffineScript (.res → .bs.js)
...
=== Method 4: Guix Flakes (Reproducible)

If you have Guix with flakes enabled:
...
=== AffineScript Compilation Failed

*Problem*: AffineScript not installed or compilation fails
...
npm install -g affinescript

# Or use the stub files (already included)
# The .bs.js stub files allow testing without AffineScript
